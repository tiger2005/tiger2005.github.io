...
尽管 Typst 在编写算法竞赛题目上存在诸多不便，不过其强大的增量编译和预览的功能，在一定程度上提供了非常好的交互体验。我可以在编写的过程中实时看到代码的运行结果，例如在一些和二维平面相关的题目中，我可以直接在预览中看到字符矩阵的变化，这对于调试和理解算法的运行过程提供了极大的帮助。（这也许是 Typst 写算法竞赛题目唯一的优势？）
