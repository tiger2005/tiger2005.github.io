...
在教程关卡中，我们已经知道如何处理一个自然数加零的情况（`add_zero`），以及一个自然数加上另一个自然数的后继的情况（`add_succ`）。为了证明零加上一个自然数等于该自然数，我们可以使用归纳法来证明这个定理。
...
这个定理主要是为了解决使用加法交换律时无法快速交换后两个数字的问题。为了证明这个问题，可以先分别使用结合律将目标的两侧重写为 `a + (b + c)` 和 `a + (c + b)`，然后使用加法交换律将 `c + b` 替换为 `b + c`。完整的代码如下：
